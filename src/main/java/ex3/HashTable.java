...
public class HashTable {
    ...
    private HashEntry[] entriesExtended;
    ...
    public void put(String key, Object value) {
        ...
        if (entries[hash] == null) {
            ...
            /* Como ahora no hay colisiones, en cuanto hay un hueco libre se añade el HashEntry, por lo que ahora la variable
            * ITEMS se debe incrementar en este momento */
            this.ITEMS++;
        } else {
            ...
            * valor del elemento existente con el valor del nuevo elemento y salimos del método. */
            ...
            copyToExtendedHashTable(entries);
            entries = entriesExtended;

            /*Después de crear la nueva tabla y rellenarla con el nuevo método copyToNewHashTable,
            * hacemos una llamada recursiva al método put(), que volverá a comprobar
            * si la nueva posición es nula y, si no, volverá a crear una nueva tabla.*/
            put(key, value);
            //temp.next = hashEntry;
            //hashEntry.prev = temp;
        }
    ...
    /*Método que copia los HashEntry de la tabla original en una nueva tabla extendida. */
    public void copyToExtendedHashTable(HashEntry[] entries) {
        entriesExtended = new HashEntry[SIZE];
        for (HashEntry hashEntry : entries) {
            HashEntry entry = hashEntry;
            while(entry != null) {
                int newHash = getHash(entry.key);
                entriesExtended[newHash] = entry;
                entry = entry.next;
            }
        }
    }
